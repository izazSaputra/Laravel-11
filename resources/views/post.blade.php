...
<main class="pt-8 pb-16 lg:pt-16 lg:pb-24 bg-white dark:bg-gray-900 antialiased border rounded-2xl backdrop-blur-sm">
              ...
              <address class="flex items-center mb-2 not-italic">
                  <div class="inline-flex items-center mr-3 text-sm text-gray-900 dark:text-white">
                      ...
                      <div>
                          ...
                          <p class="text-base text-gray-500 dark:text-gray-400"><time title="{{ $post->created_at->diffForHumans() }}">{{ $post->created_at->diffForHumans() }}</time></p>
                          <a href="/posts?category={{ $post->category->slug }}">
                            <span style="background: {{ $post->category->color }};" class="text-primary-100 text-xs font-medium inline-flex items-center px-2.5 py-0.5 rounded dark:bg-primary-200 dark:text-primary-800">
                              {{ $post->category->name }}
                          </span>   
                          </a>
                      </div>
